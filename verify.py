...
parser.add_argument('--batch-size', default=128, type=int, help='val batch size') # 512
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(val_dataloader), desc='Validate') as t:
        for image, label in val_dataloader:
            image, label = image.to(device), label.to(device)
            result = model(image)
            top1, top5 = accuracy(result, label, topk=(1, 5))
...
# python3 verify.py --model FairNAS_A --device cuda --val-dataset-root /home/ubuntu/0_datasets/data/ILSVRC2012 --pretrained-path /home/ubuntu/2_workspace/nni_hpo/3rdparty/FairNAS/pretrained/FairNasA.pth.tar